                             ...
                             int n1, int n2, int n3)
{
  ...
  for(int tiles = 0;tiles < 64;tiles++) {
    //(rows * 1024) + (tiles * 16) + (threadIdx.x)
    //(rows * 1024) = which row we belong to. tiles * 16 will be tile stride, threadIdx.x will tell us which column we are at. 
    A[threadIdx.y][threadIdx.x] = a[(rows * 1024) + (tiles * 16) + (threadIdx.x)];
    B[threadIdx.y][threadIdx.x] = b[((tiles*16) + threadIdx.y)*1024 + cols];
    __syncthreads();
    for(int k = 0;k<16;k++) {
